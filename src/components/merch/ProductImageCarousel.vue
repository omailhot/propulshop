  ...
  <div :class="cn('relative overflow-hidden rounded-xl', className)">
    ...
    <template v-if="imageCount > 1">
      <Button
        variant="secondary"
        size="icon"
        class="bg-background/85 absolute top-1/2 left-2 h-7 w-7 -translate-y-1/2 rounded-full"
      ...
      <Button
        variant="secondary"
        size="icon"
        class="bg-background/85 absolute top-1/2 right-2 h-7 w-7 -translate-y-1/2 rounded-full"
      ...
      <div
        class="bg-background/80 absolute bottom-2 left-1/2 flex -translate-x-1/2 items-center gap-1 rounded-full px-2 py-1"
      >
        <span
...
const activeIndex = computed(() =>
  imageCount.value > 0 ? index.value % imageCount.value : 0,
);
